...
importScripts(
  ...
  'background/steps/team-invite.js',
  'background/steps/team-remove.js',
  'data/names.js',
  'hotmail-utils.js',
  'microsoft-email.js',
  'luckmail-utils.js',
...
function normalizePersistentSettingValue(key, value) {
  switch (key) {
    ...
    case 'teamInviteEnabled':
    case 'hotmailFissionEnabled':
      return Boolean(value);
...
function buildOutlookFissionEmail(baseEmail = '') {
  const parts = parseEmailAddressParts(baseEmail);
  if (!parts) return baseEmail || '';
  const baseLocal = parts.local.split('+')[0];
  const suffix = generateRandomSuffix(6);
  return `${baseLocal}+${suffix}@${parts.domain}`;
}
...
const teamInviteExecutor = self.MultiPageBackgroundTeamInvite?.createTeamInviteExecutor({
  addLog,
  completeNodeFromBackground,
  getState,
  sleepWithStop,
