    ...
    async function handleMessage(rawMessage, sender) {
      ...
      switch (message.type) {
        ...
        case 'SAVE_SETTING': {
          ...
          const teamInviteChanged = Object.prototype.hasOwnProperty.call(updates, 'teamInviteEnabled')
            && Boolean(currentState?.teamInviteEnabled) !== Boolean(updates.teamInviteEnabled);
        ...
        case 'DISCOVER_TEAM_WORKSPACES': {
          const teamState = await getState();
          const teamAccessToken = String(teamState?.teamAccessToken || '').trim();
          if (!teamAccessToken) {
            return { error: '未配置 Team access token' };
          }
          try {
            ...
            if (!resp.ok) {
              return { error: 'Team token 无效或已过期 (HTTP ' + resp.status + ')' };
            }
            ...
            return { error: '获取工作区失败：' + (err?.message || String(err)) };
          }
        }
