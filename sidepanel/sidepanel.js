...
const inputTeamInviteEnabled = document.getElementById('input-team-invite-enabled');
const rowTeamBypassTokens = document.getElementById('row-team-bypass-tokens');
const inputTeamAccessToken = document.getElementById('input-team-access-token');
...
function getStepDefinitionsForMode(plusModeEnabled = false, options = {}) {
  ...
  if (typeof inputTeamInviteEnabled !== 'undefined' && inputTeamInviteEnabled) {
    requestOptions.teamInviteEnabled = Boolean(inputTeamInviteEnabled.checked);
  } else if (typeof options !== 'string' && options?.teamInviteEnabled !== undefined) {
    requestOptions.teamInviteEnabled = Boolean(options.teamInviteEnabled);
  }
...
function rebuildStepDefinitionState(plusModeEnabled = false, options = {}) {
  ...
  stepDefinitions = getStepDefinitionsForMode(currentPlusModeEnabled, {
    ...
    signupMethod: currentSignupMethod,
    phoneSignupReloginAfterBindEmailEnabled: currentPhoneSignupReloginAfterBindEmailEnabled,
    teamInviteEnabled: options?.teamInviteEnabled,
  });
...
function collectSettingsPayload() {
  ...
  return {
    ...
    step6CookieCleanupEnabled: typeof inputStep6CookieCleanupEnabled !== 'undefined' && inputStep6CookieCleanupEnabled
      ...
      : false,
    teamInviteEnabled: inputTeamInviteEnabled ? Boolean(inputTeamInviteEnabled.checked) : false,
...
function syncStepDefinitionsForMode(plusModeEnabled = false, plusPaymentMethodOrOptions = {}, maybeOptions = {}) {
  ...
  const nextTeamInviteEnabled = Boolean(inputTeamInviteEnabled?.checked);
  ...
  const shouldRender = Boolean(options.render)
    ...
    || paymentTitleChanged
    || nextTeamInviteEnabled !== Boolean(latestState?.teamInviteEnabled);
  if (!shouldRender) {
    return;
  }
  ...
  rebuildStepDefinitionState(nextPlusModeEnabled, {
    ...
    signupMethod: nextSignupMethod,
    phoneSignupReloginAfterBindEmailEnabled: nextPhoneSignupReloginAfterBindEmailEnabled,
    teamInviteEnabled: nextTeamInviteEnabled,
  });
  renderStepsList();
}
...
function applySettingsState(state) {
  if (typeof syncStepDefinitionsForMode === 'function') {
    ...
    syncStepDefinitionsForMode(stepDefinitionState.plusModeEnabled, {
      ...
      teamInviteEnabled: Boolean(state?.teamInviteEnabled),
    });
  }
  ...
  if (inputTeamInviteEnabled) {
    inputTeamInviteEnabled.checked = Boolean(state?.teamInviteEnabled);
  }
  if (inputTeamAccessToken) {
    inputTeamAccessToken.value = String(state?.teamAccessToken || '');
  }
  if (inputTeamRefreshToken) {
    inputTeamRefreshToken.value = String(state?.teamRefreshToken || '');
  }
...
inputTeamInviteEnabled?.addEventListener('change', () => {
  updateTeamBypassVisibility();
  syncStepDefinitionsForMode(currentPlusModeEnabled, getSelectedPlusPaymentMethod(), {
    render: true,
    signupMethod: currentSignupMethod,
    plusAccountAccessStrategy: currentPlusAccountAccessStrategy,
  });
...
btnToggleTeamAccessToken?.addEventListener('click', () => {
  if (!inputTeamAccessToken) return;
  const isPassword = inputTeamAccessToken.type === 'password';
  inputTeamAccessToken.type = isPassword ? 'text' : 'password';
  if (btnToggleTeamAccessToken) {
    btnToggleTeamAccessToken.textContent = isPassword ? '🔒' : '👁';
  }
});

btnDiscoverTeamWorkspaces?.addEventListener('click', async () => {
  if (!btnDiscoverTeamWorkspaces || !inputTeamWorkspaceId) return;
  btnDiscoverTeamWorkspaces.disabled = true;
  btnDiscoverTeamWorkspaces.textContent = '...';
  try {
    const response = await chrome.runtime.sendMessage({ type: 'DISCOVER_TEAM_WORKSPACES' });
    if (response?.error) {
    ...
    if (workspaces.length === 0) {
      alert('未找到 Team 工作区。请检查 access token 是否有效。');
      return;
    }
    if (workspaces.length === 1) {
      inputTeamWorkspaceId.value = workspaces[0].id || '';
      markSettingsDirty(true);
      saveSettings({ silent: true }).catch(() => { });
      return;
    }
    const choice = prompt(
      '找到多个 Team 工作区：\n' + workspaces.map((w, i) => `${i + 1}. ${w.name} (${w.id})`).join('\n') + '\n\n请输入序号选择：'
    );
    const index = Math.max(0, Math.min(workspaces.length - 1, Math.floor(Number(choice) || 1) - 1));
    inputTeamWorkspaceId.value = workspaces[index].id || '';
    markSettingsDirty(true);
    saveSettings({ silent: true }).catch(() => { });
  } catch (err) {
    alert('发现工作区失败：' + (err?.message || String(err)));
  } finally {
    btnDiscoverTeamWorkspaces.disabled = false;
    btnDiscoverTeamWorkspaces.textContent = '发现';
  }
});
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  switch (message.type) {
    ...
    case 'DATA_UPDATED': {
      ...
      if (message.payload.teamInviteEnabled !== undefined && inputTeamInviteEnabled) {
        inputTeamInviteEnabled.checked = Boolean(message.payload.teamInviteEnabled);
        updateTeamBypassVisibility();
      }
      if (message.payload.teamAccessToken !== undefined && inputTeamAccessToken) {
        inputTeamAccessToken.value = String(message.payload.teamAccessToken || '');
      }
      if (message.payload.teamRefreshToken !== undefined && inputTeamRefreshToken) {
        inputTeamRefreshToken.value = String(message.payload.teamRefreshToken || '');
      }
      if (message.payload.teamWorkspaceId !== undefined && inputTeamWorkspaceId) {
        inputTeamWorkspaceId.value = String(message.payload.teamWorkspaceId || '');
      }
